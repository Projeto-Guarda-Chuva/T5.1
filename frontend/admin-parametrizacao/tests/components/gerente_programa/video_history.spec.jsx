import { render, screen, within } from "@testing-library/react";
import { beforeEach, describe, expect, it, vi } from "vitest";
import VideoHistory from "../../../src/pages/VideoHistory";
import { formatDateTime } from "../../../src/utils/format";
import { registerCase } from "../../support/caseLog";

const useOperationLogMock = vi.fn();

vi.mock("../../../src/hooks/useOperationLog", () => ({
  default: () => useOperationLogMock(),
}));

describe("Gerente de Programa - Histórico da Operação", () => {
  beforeEach(() => {
    useOperationLogMock.mockReset();
  });

  it("lista registros de log com data, participante e status", () => {
    useOperationLogMock.mockReturnValue({
      operationLogs: {
        items: [
          {
            id: "sess-001",
            occurred_at: "2026-07-01T09:00:00Z",
            duration_seconds: 42,
            participant_email: "participante@example.com",
            status: "success",
            status_text: "Concluído",
            description: "Captura finalizada",
          },
        ],
        total: 1,
        message: "ok",
      },
      isLoading: false,
      error: null,
    });

    const caseLog = registerCase({
      input: {
        totalLogs: 1,
      },
      expected: {
        sessionId: "sess-001",
        participantEmail: "participante@example.com",
        statusText: "Concluído",
        occurredAt: formatDateTime("2026-07-01T09:00:00Z"),
      },
    });

    render(<VideoHistory />);

    expect(screen.getByText("sess-001")).toBeInTheDocument();
    expect(screen.getByText("participante@example.com")).toBeInTheDocument();
    expect(screen.getAllByText("Concluído")).toHaveLength(2);
    expect(
      screen.getByText(formatDateTime("2026-07-01T09:00:00Z")),
    ).toBeInTheDocument();

    caseLog.output = {
      renderedSessionId: screen.getByText("sess-001").textContent,
      renderedDate: screen.getByText(formatDateTime("2026-07-01T09:00:00Z"))
        .textContent,
      renderedStatus: screen.getAllByText("Concluído")[0].textContent,
    };
  });

  it("deve informar claramente quando não existem registros de log", () => {
    useOperationLogMock.mockReturnValue({
      operationLogs: {
        items: [],
        total: 0,
        message: "Nenhum log encontrado.",
      },
      isLoading: false,
      error: null,
    });

    const caseLog = registerCase({
      input: {
        totalLogs: 0,
        message: "Nenhum log encontrado.",
      },
      expected: {
        emptyStateMessage: "Nenhum log encontrado.",
      },
      notes: [
        "Contrato de usabilidade: a ausência de histórico deve ser explicitamente comunicada.",
      ],
    });

    render(<VideoHistory />);

    caseLog.output = {
      renderedText: document.body.textContent,
    };

    expect(screen.getByText("Nenhum log encontrado.")).toBeInTheDocument();
  });

  it("renderiza o ramo visual de erro quando o status do log não é success", () => {
    useOperationLogMock.mockReturnValue({
      operationLogs: {
        items: [
          {
            id: "sess-002",
            occurred_at: "2026-07-01T10:15:00Z",
            duration_seconds: 15,
            participant_email: "falha@example.com",
            status: "error",
            status_text: "Falhou",
            description: "Erro ao salvar vídeo",
          },
        ],
        total: 1,
        message: "ok",
      },
      isLoading: false,
      error: null,
    });

    const caseLog = registerCase({
      input: {
        totalLogs: 1,
        status: "error",
      },
      expected: {
        sessionId: "sess-002",
        statusText: "Falhou",
        statusClass: "text-danger",
      },
    });

    const { container } = render(<VideoHistory />);

    const errorStatusText = screen.getAllByText("Falhou")[0];
    const errorIconWrapper = container.querySelector(".bg-danger.text-danger");

    expect(screen.getByText("sess-002")).toBeInTheDocument();
    expect(screen.getAllByText("Falhou")).toHaveLength(2);
    expect(errorStatusText).toHaveClass("text-danger");
    expect(errorIconWrapper).not.toBeNull();

    caseLog.output = {
      renderedStatus: errorStatusText.textContent,
      statusClass: errorStatusText.className,
      hasDangerWrapper: Boolean(errorIconWrapper),
    };
  });

  it("deve refletir no rodapé a quantidade real de registros retornados", () => {
    useOperationLogMock.mockReturnValue({
      operationLogs: {
        items: [
          {
            id: "sess-003",
            occurred_at: "2026-07-01T11:00:00Z",
            duration_seconds: 10,
            participant_email: "um@example.com",
            status: "success",
            status_text: "Concluído",
            description: "ok",
          },
        ],
        total: 1,
        message: "ok",
      },
      isLoading: false,
      error: null,
    });

    const caseLog = registerCase({
      input: {
        totalLogs: 1,
      },
      expected: {
        footerText: "Exibindo 1 registro",
      },
      notes: [
        "Contrato esperado: o rodapé deve refletir a quantidade retornada pelo backend, sem valor fixo hardcoded.",
      ],
    });

    render(<VideoHistory />);

    caseLog.output = {
      pageText: document.body.textContent,
    };

    expect(screen.getByText(/Exibindo 1 registro/)).toBeInTheDocument();
  });
});
