...
describe("Gerente de Programa - Página de Parâmetros", () => {
  ...
  it("deve classificar erro 4xx de ativação como erro no servidor externo da aplicação", async () => {
    const user = userEvent.setup();
    ...
    const caseLog = registerCase({
      ...
      expected: {
        errorMessage: "Erro no servidor externo da aplicação",
      },
      notes: [
        "Contrato esperado: rejeição 4xx da integração inferior deve ser comunicada como falha da aplicação externa.",
      ],
    });
    ...
    const renderedMessage = await screen.findByText(
      "Erro no servidor externo da aplicação",
    );
    caseLog.output = {
    ...
    expect(
      screen.getByText("Erro no servidor externo da aplicação"),
    ).toBeInTheDocument();
  });

  it("deve classificar erro não 4xx de ativação como erro no servidor externo da aplicação", async () => {
    const user = userEvent.setup();
    useActivateConfigurationMock.mockImplementation((options) => ({
      activateConfiguration: vi.fn(async () => {
        await options?.onError?.({ status: 503 });
      }),
      isPending: false,
    }));
  ...
  it("mostra erro interno quando a criação de configuração falha", async () => {
    const user = userEvent.setup();
    useCreateConfigurationMock.mockImplementation((options) => ({
      createConfiguration: vi.fn(async () => {
        await options?.onError?.(new Error("create failed"));
      }),
      isPending: false,
    }));
    ...
    const caseLog = registerCase({
      input: {
        createConfigurationThrows: "create failed",
      },
      expected: {
        errorMessage: "Ocorreu um erro no servidor interno",
      },
    });
    ...
    await user.click(screen.getByRole("button", { name: /salvar modal de criação/i }));

    const renderedMessage = await screen.findByText(
      "Ocorreu um erro no servidor interno",
    );
